...
const verifyGuildMember = async (req:AuthedRequest, res:Response, next:NextFunction) => {try {
    ...
    // Log & return error:
    logtail.warn(`[🔑] FAILED to verify guild membership for API request - See details...`, {err, user: req?.user, guildId: req?.params?.guildId});
...
const verifyGuildAdmin = async (req:AuthedRequest, res:Response, next:NextFunction) => {try {
    // Get user & guild from request:
    const guildId = req.params?.guildId;
    const userId = req?.user?.id;
    if(!guildId) return new APIResponse(res).sendFailure(`Bad Request - Couldn't verify guild membership, a guild id was unprovided.`, 400);
