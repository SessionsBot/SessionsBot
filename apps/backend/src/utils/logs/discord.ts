...
const joinLeaveLogChannelId = ENVIRONMENT_TYPE == 'production'
    ? '1424195270618644655'
    : '1462313668678258739';
const entitlementsLogChannelId = ENVIRONMENT_TYPE == 'production'
    ? '1425618618539835432'
    : '1462313734847598747';
    ...
    events: {
        ...
        entitlementCreated: async (entitlement: Entitlement) => {
            try {
                ...
                const logGuild = await core.botClient.guilds.fetch(logGuildId);
                if (!logGuild) throw `Failed to fetch guild for logging event`
                const logChannel = await logGuild.channels.fetch(entitlementsLogChannelId)
                if (!logChannel || !logChannel.isSendable()) throw `Failed to fetch channel for logging event`
