...
export default {
    ...
    events: {
        ...
        guildRemoved: async (guildRemoved: Guild, wasSetup: boolean) => {
            try {
        ...
        guildSetup: async (guildId, configuration: { accentColor: string, allGuildSchedules: any, panelChannelId, dailySignupPostTime, signupMentionIds, timeZone }) => {
            try {
                ...
                createLog.for('Bot').warn(`[?] Failed to post event "Guild Setup" to internal Discord Log.`, { err })
            }
        },
