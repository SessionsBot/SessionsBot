...
export default async (guildId:string, messageContent:ContainerBuilder) => {try {
    // Fetch guild:
    const guild = await core.botClient.guilds.fetch(guildId);

    // Attempt to send in "System" channel:
    try { if(guild.systemChannel && guild.systemChannel.isSendable()) {
        await guild.systemChannel.send({
            components: [messageContent],
            flags: MessageFlags.IsComponentsV2
        })
        return new SuccessResult(`Message was sent to system channel!`);
    }} catch(err) { }
    ...
    try {
        ...
        for(const [, channel] of fallbackChannels) {
            ...
            // If succeeded:
            return new SuccessResult(`Message was sent through a fallback channel! - Id: ${channel?.id}`);
        }
    } catch (err) { }
    ...
    try {
        ...
        // Return success:
        return new SuccessResult(`Message was sent to server owner!`);
    } catch (err) {
    ...
    return new ErrorResult('Failed to desperately send message to  guild...', err);
}}