...
<template>
    <Paginator v-model:first="pageIndexStart" :total-records="props?.totalRecords" :rows="props?.pageSize"
        :always-show="props?.alwaysShow" class="paginator">
        <template
            ...
            <div class="flex-center gap-2 w-full flex-wrap p-2">
                ...
                <span class="text-text-1/75 flex-center gap-1.5">
                    ...
                    <Select size="small" :model-value="page + 1" :options="selectPageOptions(pageCount ?? 0)"
                        ...
                        :pt="{ root: 'bg-bg-soft!', list: 'bg-bg-soft! rounded! text-sm!', label: 'p-1! pl-2! w-fit!', option: '[.p-select-option-selected]:bg-brand-1! flex-center! p-1.75! selected-bg-emerald-500! checked:bg-yellow-500!', dropdown: 'pl-1! pr-2! m-0! w-fit!' }" />
                    of {{ pageCount }}
                </span>
                <!-- Next -->
