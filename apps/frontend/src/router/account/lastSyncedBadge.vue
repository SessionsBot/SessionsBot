    ...
    async function attemptDataResync() {
        ...
        if (!result.success) {
            ...
            if (result.data.reason == 'COOLDOWN') {
                // Reason - Cooldown:
                return toaster.update(thisToast, { content: result.data.message, options: { type: TYPE.ERROR, timeout: 7_000, closeOnClick: true, } })
            } else {
                // Reason - Other/Unknown:
                return toaster.update(thisToast, { content: result.data.message, options: { type: TYPE.ERROR, timeout: 10_000, closeOnClick: true, } })
            }
        } else {
            // Succeeded:
            return toaster.update(thisToast, { content: 'Success! Your account data has been refreshed with Discord.', options: { type: TYPE.SUCCESS, timeout: 5_000, closeOnClick: true, } })
        }
    }
...
<template>
    <div class="text-xs mt-2 flex flex-row flex-wrap items-center content-center justify-center">
        ...
        <span
            ...
            <i class="pi pi-discord drop-shadow-sm drop-shadow-black  scale-90 ml-1 mr-0.5" />
            <p class="mr-0.5 drop-shadow-sm drop-shadow-black font-semibold "> Last Synced</p>
        </span>
        ...
        <div class="flex flex-row gap-0 h-7 flex-nowrap rounded-r-md overflow-clip transition-all" :class="{
            ...
            'bg-red-700/50! hover:bg-red-700/50! active:bg-red-700/50!': (auth.$state.refreshStatus == 'failed'),
            'bg-green-700/50! hover:bg-green-700/50! active:bg-green-700/50!': (auth.$state.refreshStatus == 'succeeded'),
        }">
            ...
            <Button unstyled title="Refresh Account Data"
                class="px-1 border-2 border-l borderColor   flex items-center justify-center cursor-pointer bg-zinc-500/50 hover:bg-zinc-600/60 active:bg-zinc-500/70 disabled:cursor-not-allowed h-full transition-all"
